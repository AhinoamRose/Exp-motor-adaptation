...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='C:\\Users\\אחינעם רוזנצויג\\Documents\\פרוייקט 2023\\Psy\\Experiment_Psy\\\u200f\u200fmyVersion_exp_lastrun.py',
...
def run(expInfo, thisExp, win, inputs, globalClock=None, thisSession=None):
    ...
    for thisTrial in trials:
        ...
        def test_movement_started():
            return euclidean_dist(rotated_move_mouse.getPos(), rotated_fixation_2.pos) > 0.5
        
        def test_movement_ended(mouseDist, movement_duration):
            if  mouseDist < 0.1 or movement_duration > 1:
                movement_ended = True
            else:
                movement_ended = False
        ...
        while continueRoutine:
            ...
            # Loop for the remaining beeps
            if ty=='preparation':
                while beep_counter < 4:
                    t = routineClock.getTime()
                   
                    # Check if it's time for the next beep
                    if t >= beep_counter * interval:
                        beep.play()
                        beep_counter += 1
            
                    if not target_displayed and t >= target_time:
                        print("*",point_x, point_y)
                        rotated_target_circle_2.draw()
                        print("**",point_x, point_y)
                        rotated_fixation_2.draw()
                        print("***",point_x, point_y)
                        mouse_circle_2.draw()
                        print("****",point_x, point_y)
